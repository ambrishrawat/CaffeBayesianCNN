...
def fast_sgd_fullback(dbtype = 'leveldb', dbno = 1):
	...
	for gstep in xrange(2):
		...
		input_grads = cnn2.get_data_grads(input_fool,prob_ap1)
		#propagate grads through the transform
		input_grads = utils.backward_T(input_grads,inv_P_,mean_)
		
		cnn.save_img_ind(input_orig, path = src_path+'/exp2/fast_sgd_noise', tag = str(gstep), tr = False)
	...
	np.save(src_path+'/exp2'+'/results/nfbnodrop_prob',c_prob1)
	np.save(src_path+'/exp2'+'/results/nfballdrop_ap_prob',c_prob2)
	np.save(src_path+'/exp2'+'/results/nfbfcdrop_ap_prob',c_prob3)
	np.save(src_path+'/exp2'+'/results/nfballdrop_prob',mc_prob2)
	np.save(src_path+'/exp2'+'/results/nfbfcdrop_prob',mc_prob3)
	np.save(src_path+'/exp2'+'/results/nfbimg_adv',img_adv)
...
def exp_adv(model = 'nodrop', dbtype = 'leveldb', dbno = 1, mode = 'trial'):
	...
	for gstep in xrange(grad_steps):
		...
		Step 1: Set the data for the network for which you want the adversarial image
		'''
		prob = cnn2.get_det_probs(img_set=input_fool)
		c_prob2[:,gstep,:] = prob.copy()
		...
		'''
		input_grads = cnn2.get_data_grads(input_fool,prob)
		img_adv[:,gstep,:,:,:] = input_fool.copy()
		#print 'NONZERO GRADS: ', np.count_nonzero(input_grads), '\t GSTEPS ',gstep
		input_fool -= input_grads*2.e-3
		cnn2.save_img_ind(input_fool, path = src_path+'/exp2', tag = str(gstep))
		
	'''
	save the images 
	'''
	np.save(src_path+'/exp2'+'/results/nodrop_prob',c_prob1)
	np.save(src_path+'/exp2'+'/results/alldrop_ap_prob',c_prob2)
	np.save(src_path+'/exp2'+'/results/fcdrop_ap_prob',c_prob3)
	np.save(src_path+'/exp2'+'/results/alldrop_prob',mc_prob2)
	np.save(src_path+'/exp2'+'/results/fcdrop_prob',mc_prob3)
	np.save(src_path+'/exp2'+'/results/img_adv',img_adv)
